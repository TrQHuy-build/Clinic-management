document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", function (e) {
        ...
        // Before submit: set combined appointment datetime (ISO) into hidden field
        console.log("=== FORM SUBMIT DEBUG ===");
        console.log("selectedDate:", selectedDate);
        console.log("time:", time);
        
        if (selectedDate && time) {
            // Build ISO datetime using selectedDate and time (HH:mm)
            const [hour, minute] = time.split(":");
            const dt = new Date(selectedDate.getTime());
            dt.setHours(parseInt(hour, 10), parseInt(minute, 10), 0, 0);
            const isoDate = dt.toISOString();
            console.log("Setting appointment_date to:", isoDate);
            document.getElementById("appointment_date").value = isoDate;
        } else {
            console.error("Missing selectedDate or time!");
        }

        // Create FormData AFTER setting appointment_date
